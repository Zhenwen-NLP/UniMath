...
import nltk
from data_utils import process_image, create_patch, process_english_text, get_input_tablemwp, transfer_num_no_tokenize, load_raw_data, table_loader
import re
import pandas as pd
...
class GeoDataset(Dataset):
    def __init__(self, split='train', raw_dataset=None, verbose=True, args=None, mode='train'):
        ...
        if 'calculation' in str(self.source):
            ...
            if 'train' in split:
                mwp_pairs, _, _, _ = transfer_num_no_tokenize(train_data, test_data, False)
            else:
                _, mwp_pairs, _, _ = transfer_num_no_tokenize(train_data, test_data, False)
            
            for pair in mwp_pairs:
                datum = {
                    'image': None,
                    'source_text': deepcopy('Math word problem solving with number mapping: ' + ' '.join(pair[0]) + ' Answers: '),
            ...
            for pid in pids:
                datum = {
                    'image': None,
                    'source_text': 'Math word problem solving without mapping: ' + deepcopy(get_input_tablemwp(problems[pid])) + ' Answers: ',
            ...
            with open(geo_dir.joinpath(f'{source}.pk'), "rb") as f:
                ...
                for sample in dataset:
                    if 'calculation' in source:
                        ...
                        problem_with_space = 'Geometry calculation with number mapping: ' + problem_with_space + ' Answers: '
                        ...
                        problem_with_space = 'Geometry proving with mapping: ' + problem_with_space + ' Answers: '
