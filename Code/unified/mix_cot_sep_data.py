    ...
    def __init__(self, split='train', raw_dataset=None, verbose=True, args=None, mode='train'):
        ...
        if 'calculation' in str(self.source):
            ...
            for pair in mwp_pairs:
                datum = {
                    'image': None,
                    'source_text': deepcopy('Math word problem solving with mapping: ' + ' '.join(pair[0])),
            ...
            for pid in pids:
                if 'train' in split:
                    datum = {
                        'image': None,
                        'source_text': 'Math word problem chain of thought: ' + deepcopy(get_input_tablemwp(problems[pid])),
                        'target_text': deepcopy(problems[pid]['solution']),
                        'source_nums': deepcopy(problems[pid]['unit']),
                        'choice_nums': deepcopy(problems[pid]['choices']),
                        'label': deepcopy(problems[pid]['answer']),
                        'problem_form': 'table_mwp_cot',
                        'problem_type': None
                    }
                    data.append(datum)
                datum = {
            ...
            with open(geo_dir.joinpath(f'{source}.pk'), "rb") as f:
                ...
                for sample in dataset:
                    if 'calculation' in source:
                        ...
                        problem_with_space = 'Geometry calculation with mapping: ' + problem_with_space
