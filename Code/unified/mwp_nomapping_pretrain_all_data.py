...
class GeoDataset(Dataset):
    def __init__(self, split='train', raw_dataset=None, verbose=True, args=None, mode='train'):
        ...
        self.tokenizer = VLT5TokenizerFast.from_pretrained(
            args.backbone,
            do_lower_case=self.args.do_lower_case, use_auth_token = False)
        ...
        if 'calculation' in str(self.source):
            ...
            for pair in mwp_pairs:
                ...
                if r > self.args.r_threshold:
                    ...
                    source_text, target_text = preprocess.corrupt_bart(
                            deepcopy(' '.join(pair[0])), mask_ratio=self.args.word_mask_rate, prefix='Denoise text: ')
                    datum = {
                        'image': None,
            ...
            for pid in pids:
                ...
                if r > self.args.r_threshold:
                    ...
                    source_text, target_text = preprocess.corrupt_bart(
                           deepcopy(get_input_tablemwp(problems[pid])), 
                           mask_ratio=self.args.word_mask_rate, prefix='Denoise text: ')
                    datum = {
                        'image': None,
                        'source_text': source_text,
                        'target_text': target_text,
                ...
                for sample in dataset:
                    ...
                    if 'calculation' in source:
                        ...
                        if r > self.args.r_threshold:
                            ...
                            text_i = " ".join(sample["manual_program"])
                            source_text, target_text = preprocess.corrupt_bart(
                                problem_with_space, mask_ratio=self.args.word_mask_rate, prefix='Denoise text: ')
